        ...
        for cv_iter in range(1):
            for init_model in ['imagenet']:
                ...
                for database_name in data_split['database_names']:
                    ...
                    accs,aucs,dices,tps,fps,fns,tns = test_fcn_vessels('../' + config.method + '/' + database_name + str(cv_iter) + init_model, config, universal_model_name, tmp_test)
                    ...
                    accs,aucs,dices,tps,fps,fns,tns = test_fcn_vessels('../' + config.method + '/' + database_name + str(cv_iter)  + init_model, config, model_name, tmp_test)
                    ...
                    model_name = train(config,data_train=tmp_train,data_valid=tmp_valid)
                    accs,aucs,dices,tps,fps,fns,tns = test_fcn_vessels(config.results_folder + '/' + config.method + '/' + database_name + str(cv_iter)  + init_model, config, model_name, tmp_test)
                ...
                with open(config.results_folder + '/result_new_loader_1000_' + str(cv_iter)  + init_model + '_a_pretrained.json', 'w') as outfile:
